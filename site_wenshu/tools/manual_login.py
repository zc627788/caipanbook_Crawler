...
def run_auto_login():
    with sync_playwright() as p:
        ...
        try:
            ...
            if all_cookies.get("wzws_reurl", {}).get("value"):
                s2.cookies.set("wzws_reurl", all_cookies["wzws_reurl"]["value"], domain="wenshu.court.gov.cn")
            ...
            if "anonymous" in str(nm2).lower() or not nm2:
                print(f"  ❌ curl_cffi 用户状态为匿名: {r2.text[:150]}")
            else:
                print(f"  ✅ curl_cffi 用户状态验证通过: {nm2}")

            # 进一步验证文书搜索接口 queryDoc 是否能正常拿数据
            if page_id:
                now_dt = datetime.now()
                date_str = now_dt.strftime("%Y%m%d")
                cipher = encrypt_ciphertext(int(time.time() * 1000), generate_random_salt(24), date_str)
                token = generate_random_salt(24)
                cond_str = urllib.parse.quote(json.dumps([{"key": "cprq", "value": "2025-07-01 TO 2025-07-31"}], separators=(',', ':')))
                body_doc = (
                    f"pageId={page_id}&cprqStart=2025-07-01&cprqEnd=2025-07-31&"
                    f"sortFields=s50%3Adesc&pageNum=1&pageSize=5&queryCondition={cond_str}&"
                    f"cfg=com.lawyee.judge.dc.parse.dto.SearchDataDsoDTO%40queryDoc&wh=960&ww=1536&cs=0&"
                    f"__RequestVerificationToken={token}&ciphertext={urllib.parse.quote(cipher)}"
                )
                r_doc = s2.post(
                    "https://wenshu.court.gov.cn/website/parse/rest.q4w",
                    headers={
                ...
                res_doc_json = r_doc.json()
                if res_doc_json.get("code") == 1:
                    dec_txt = decrypt_result(res_doc_json["result"], res_doc_json["secretKey"], date_str)
                    dec_json = json.loads(dec_txt)
                    print(f"  🎉 curl_cffi 文书检索接口(queryDoc)验证成功! 返回文书总数: {dec_json.get('queryResult', {}).get('resultCount', 0)}")
                else:
                    print(f"  ❌ curl_cffi 文书检索接口(queryDoc)返回异常: {res_doc_json}")
        except Exception as e:
            print(f"  ❌ curl_cffi 验证异常: {e}")
